...
function renderFileChips(files) {
  return (files || []).map((f, index) => `
    <span class="file-chip" data-file-path="${escapeAttr(f.path)}" data-file-name="${escapeAttr(f.name)}">
...
function persistDealField(dealId, field, value) {
  debounceSave(`deal:${dealId}:${field}`, async () => {
...
async function uploadDealFile(dealId, file) {
  ...
  const path = `${state.user.id}/${deal.cycle_id}/${deal.id}/${Date.now()}-${safeName}`;
  const { error } = await supabase.storage.from('client-files').upload(path, file, { upsert: false, contentType: file.type || 'application/octet-stream' });
  if (error) throw error;
  const files = [...(deal.files || []), { name: file.name, path }];
...
async function openDealFile(path) {
...
async function removeDealFile(dealId, index) {
  ...
  if (!removed) return;
  await supabase.storage.from('client-files').remove([removed.path]);
...
function bindEvents() {
  ...
  el('refreshDataBtn').addEventListener('click', async () => {
    try { await loadData(); showToast('Данные обновлены.'); } catch (err) { showToast(err.message, true); }
  });
  ...
  el('dealsTbody').addEventListener('change', async (e) => {
    ...
    if (e.target.matches('[data-upload]')) {
      ...
      if (!file) return;
      try { await uploadDealFile(tr.dataset.id, file); showToast('Файл загружен.'); } catch (err) { showToast(err.message, true); }
      e.target.value = '';
    }
  });
  ...
  el('dealsTbody').addEventListener('keydown', (e) => {
    if (e.key === 'Enter' && (e.target.matches('input[data-field]') || e.target.matches('textarea[data-field]'))) {
      e.preventDefault();
      persistDealField(e.target.closest('tr[data-id]').dataset.id, e.target.dataset.field, e.target.value);
  ...
  el('dealsTbody').addEventListener('click', async (e) => {
    ...
    if (action === 'open-file') {
      ...
      try { await openDealFile(chip.dataset.filePath); } catch (err) { showToast(err.message, true); }
    }
