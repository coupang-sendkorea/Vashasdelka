...
async function uploadDealFile(dealId, file) {
  ...
  try {
    ...
    if (error) throw error;
    fileRecord = { name: file.name, path, mode: 'storage', mimeType: file.type || '', size: file.size || 0 };
  } catch (storageError) {
    console.warn('Storage upload failed, fallback to inline DB storage', storageError);
    if ((file.size || 0) > 4 * 1024 * 1024) {
      throw new Error('Не удалось сохранить файл в облачное хранилище. Для встраивания в базу файл должен быть не больше 4 МБ.');
    }
    const inlineData = await readFileAsDataUrl(file);
    fileRecord = { name: file.name, mode: 'inline', inlineData, mimeType: file.type || '', size: file.size || 0 };
  }
...
async function removeDealFile(dealId, index) {
  ...
  if (!removed) return;
  if (removed.mode === 'storage' && removed.path) {
